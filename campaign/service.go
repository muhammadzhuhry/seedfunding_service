...
type Service interface {
	...
	CreateCampaignImage(input CreateCampaignInput, fileLocation string) (CampaignImage, error)
}
...
func (s *service) GetCampaigns(userID int) ([]Campaign, error) {
	// jika user mengirimkan user id maka kita memanggil data campaign berdasarkan  userid yguser kirim
	if userID != 0 {
...
func (s *service) CreateCampaignImage(input CreateCampaignImageInput, fileLocation string) (CampaignImage, error) {
	// check update primary image or not
	isPrimary := 0
	if input.IsPrimary {
		isPrimary = 1
